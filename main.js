...
import { makeWalletWatchTrigger, makeInboundExternalManager, bigIntReplacer } from './src/helpers.js';
import strategyConfig from './strategy.config.js';

const main = async () => {
    const timestamp = Date.now();
    const networkConfig = process.env.NETWORK_CONFIG || 'https://wallet.agoric.app/wallet/network-config';
    const instancePath = process.env.INSTANCE_PATH || 'auctioneer';
    ...
    const bidManager = makeBidManager(offerSender, instancePath);
    const externalManager = makeInboundExternalManager(stateManager.getState);
